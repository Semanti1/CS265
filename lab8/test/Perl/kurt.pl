...
print "\nThis is just a system call, we do nothing with it "
	...
	# we set up the command line here - redirect stdout/stderr to null
my @args = qq^wc $inFile &> /dev/null ^;
...
print "Okay, that was the 1st call.  stdout/stderr were redirected\n"
	."to /dev/null, so you don't see it\n"

	# to save the return value:

print "\nNow, we'll do that again, but let the output be displayed by\n"
	."the called process, and save the return value.\n";
